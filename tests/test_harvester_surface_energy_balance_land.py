...
def test_global_mean_values(tolerance=0.001):
    """The value of -1.35905391706083 is the mean value of the global means 
    ...
    these forecast files using a separate NCL(NCAR Command language) script.
    """
    data1 = harvest(VALID_CONFIG_DICT)

    global_mean = abs(-1.35905391706083) 
    for item in data1:
        if item.statistic == 'mean':
           harvested = abs(item.value)  
           assert harvested <= (1 + tolerance) * global_mean
           assert harvested >= (1 - tolerance) * global_mean

def test_gridcell_variance(tolerance=0.001):
    data1 = harvest(VALID_CONFIG_DICT)

    variance = 314.6432986219634 
    for item in data1:
        if item.statistic == 'variance':
           print("variance ",item.value) 
...
def test_gridcell_min_max(tolerance=0.001):
    ...
    offline_min = -608.3251571655273  
    offline_max = 272.9491453170776
    for item in data1:
