...
class VarStatsCatalog :
    ...
    def calculate_weighted_average(self,weights,temporal_mean):
        """This method calculates a weighted average  
           ...
           """
        twod_weights = weights.mean(axis=0)    
        spatial_weighted_sum = np.nansum(twod_weights * temporal_mean)
        sum_of_weights = np.nansum(twod_weights)
        value  = spatial_weighted_sum / sum_of_weights if sum_of_weights != 0 else np.nan
    ...
    def calculate_var_variance(self,weights,temporal_mean):
        ...
        try:
            assert twod_weights.shape == temporal_mean.shape
        ...
        spatial_weighted_sum = np.nansum(twod_weights * temporal_mean)
        sum_of_weights = np.nansum(twod_weights)
        weighted_average = spatial_weighted_sum / sum_of_weights
        value = np.nansum(sum_of_weights * (temporal_mean - weighted_average) ** 2) / sum_of_weights 
