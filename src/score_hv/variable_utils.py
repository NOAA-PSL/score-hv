...
class VarUtilsCatalog(object):
    ...
    def check_variable_exists(self,var_name):
        ...
        dataset_variable_names: A list of variables names that are in the input dataset.
        """
    ...
    def extract_variable_info(self,var_name):
        ...
        if var_name == "netef_ave":
            ...
            variable_data = self.get_gmst()
            longname = 'global mean surface temperature'
            units = 'K'
        elif var_name == 'gsat':
            variable_data = self.get_gmat()
            longname = 'global surface air temperature'
            units = 'K'
    ...
    def calculate_toa_radative_flux(self):
        ...
        The required fields are:
        dswrf_avetoa:averaged TOA downward shortwave flux
        uswrf_avetoa:averaged TOA upward shortwave flux
        ulwrf_avetoa:averaged TOA upward longwave flux
    ...
    def get_sst(self, sst_var='tmpsfc'):
        """This method reads the tmpsfc from the dataset.
        """
        self.check_variable_exists(sst_var)
        
        return(self.dataset[sst_var])

    def get_nsst(self, nsst_var='tref'):
        self.check_variable_exists(nsst_var)
        return(self.dataset[nsst_var])
        
    def get_gsat(self, gsat_var='tmp2m'):
        self.check_variable_exists(gsat_var)
        return(self.dataset[gsat_var])
        
    def get_gmst(self, gmst_vars=['tmp2m', 'tmpsfc']):
        for gmst_var in gmst_vars:
            self.check_variable_exists(gmst_var)
        #TODO: calculate GMST defined as the weighted average of near surface 
        # (2m) air temperature over sea ice and land and SST over open ocean
        # return(self.dataset[gsat_var])
